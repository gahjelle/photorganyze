...
def store(path):
    ...
    img_exists, orig_path = check_image_exists(img_hash, img_vars)
    img_vars["duplicate"] = "DUP" if img_exists else ""
    write_img_info(path, orig_path if img_exists else output_path, img_vars)
    if img_exists:
        return
...
def get_image_vars(path):
    ...
    img_vars["original"] = os.path.splitext(os.path.basename(path))[0]
    img_vars["base"] = get_original_image_name(path, date)
    img_vars["ext"] = image_format_map.get(img.format.lower(), img.format.lower())
    img_vars["height"] = str(exif.get("ExifImageHeight", -1))
    img_vars["width"] = str(exif.get("ExifImageWidth", -1))
...
def get_file_hash(path):
    ...
    return hash_bytestr_iter(
        file_as_blockiter(open(path, mode="rb")), hashlib.sha256(), True
    )
...
def check_image_exists(img_hash, img_vars):
    check_path = os.path.join(
        config.get_path("directory", "output"), config.get("checksum_file", "output")
    )
    try:
    ...
    if img_hash in img_hashes:
        ...
        return True, img_hashes[img_hash]
...
def get_output_path(img_vars):
    ...
    output_path = os.path.join(
        config.get_path("directory", "output"), config.get("file_name", "output")
    )
...
def write_img_info(path_in, path_out, img_vars):
    info_file = config.get_path("file_name", "info")
    img_vars["path_in"] = path_in
    img_vars["path_out"] = path_out
    vars_to_write = config.get_tuple("vars", "info")
    with open(info_file, mode="a") as fid:
        fid.write(";".join(img_vars[v] for v in vars_to_write) + "\n")
